...
eleven wrong predictions and the number 5 had eight wrong predictions. 
...
**Answer:** The code to create K-nearest neighbor classifiers for different $K$ and
...
$-log(0.49) \approx 0.7$.
The reason why cross-entropy might be a better choice of error function is:
misclassification error will only give us information of how often
the model is wrong, cross-entropy will give a measure of how certain the model
is on the true values.
...
The comparison result can be seen in the table above. The penalty parameter that is most appropriate is $\theta$ = 100, because it get the lowest MSE on test data.

The degrees of freedom tells us how many parameters that are not 0. 
Ridge regression with $\lambda$, will shrink all parameters towards 0. 
The larger value for $\lambda$ results in smaller degrees of freedom, since more
parameters are closer to 0. 
...
```{r, fig.cap = "\\label{} Scatterplot showing a Plasma glucose concentration on Age where observations are colored by Diabetes levels."}
...
**Answer**
Since the decision boundary from a logistic model is a linear line,
we believe that diabetes is not easy to classify by a standard logistic regression model that uses age and plasma glucose concentration as variables. They do not seem to separate the diabetes classes well.
...
```{r, fig.cap = "\\label{} Scatterplot showing a Plasma glucose concentration on Age where observations are colored by predicted values of Diabetes."}
...
If we compare this figure 6 to the figure 5 in step 3.1 we can see that the predicted values from figure 6 do not entirely align with the true values from figure 5.
...
```{r, fig.cap = "\\label{} Scatterplot showing a Plasma glucose concentration on Age where observations are colored by predicted values of Diabetes with decision boundary."}
...
The decision boundary is not able to capture the data distribution well. It is notable that the boundary between diabetes classes does not show a linear pattern in figure 5, which means that
...
```{r, fig.cap = "\\label{} Scatterplot showing a Plasma glucose concentration on Age where observations are colored by predicted values of Diabetes for different thresholds."}
...
```{r, fig.cap = "\\label{} Scatterplot showing a Plasma glucose concentration on Age where observations are colored by predicted values of Diabetes with the new features"}
...
The misclassification error is approximately 0.25.
...
The misclassification rate is around one percentage point lower than the model in 3.2, indicating a slight improvement. However, this model is more complex and the parameters are more harder to interpret than the model in 3.2, which could argue that the model in 3.2 is better.

The basis expansion trick allows us to fit a logistic regression model with a non-linear decision boundary.
...
efficient when asking questions), Duc on task 1, Simge on task 2, and William on task 3.
